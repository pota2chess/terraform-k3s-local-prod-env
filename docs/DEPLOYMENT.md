...
3. Get a token for runner: after the project is launched, go to localhost:3000 and complete registration, go to Setting -> Action -> New runner and get a token, import him to variable of terminal session (see .env.example). Run: terraform apply -replace=docker_container.gitea -replace=docker_container.runner
